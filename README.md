...
- nvim-autopairs 等、閉じ文字を自動挿入するプラグインとの相性問題（挿入位置の割り込み・カーソル位置のずれ・`z(` の全角/半角不揃い）は解決済み。詳細は前述の「nvim-autopairs との相性問題」参照。abbrev モードで見出しが空・候補0件の状態のまま記号を打鍵すると即座に確定してしまう別の相性問題も、henkan アクティブ中はオートペア側を無効化する方式で解決済み。詳細は前述の「abbrev モードでのオートペア相性問題と、推奨される回避策」参照。
...
- `<Left>`/`<Right>` などのカーソル移動キーやマウスクリックは、henkan 非アクティブ時の未確定ローマ字バッファについては安全に扱える（`is_target_key` に該当しないキーが来た時点でバッファを無条件にリセットするため、確定済みかなを破壊するようなバイト列破損は起こらない）が、**表示上の不整合**（未確定ローマ字がそのまま残る）は起こりうる。henkan（▽/▼）アクティブ中にこれらのキーが来た場合の挙動は未検証。なお、nvim-autopairs 等が生成する合成キー列中の `<Left>`/`<Right>`（バッチの flush 待ち中に限り横取りしてカーソル位置を補正する仕組み。詳細は前述の「nvim-autopairs との相性問題」参照）は別途対応済みで、この制限の対象外。
...
- **`enter_key`（`extra_candidate_next_key`/`prev_key` 以外の設定項目）でも、同じ「端末依存の符号化の揺れ」問題が起きる（実機で発見）**: `enter_key` を `<C-j>` から `<C-;>` に変更する構成を試したところ、Konsole では `<C-;>` が機能しなかった（他の端末では動作した）。Ctrl+印字可能文字（記号）の符号化が端末依存であることは前述の `<C-.>`/`<C-,>` の一件と同じ原因と考えられる。**この節で説明しているキー選定の注意点は `extra_candidate_next_key`/`extra_candidate_prev_key` に限らず、`enter_key`/`buffer_enter_key`/`cmdline_enter_key` 等、skk.nvim のあらゆるキー設定項目に共通して当てはまる**。加えて、システム側（デスクトップ環境や入力メソッドフレームワーク）がキーをグローバルショートカットとして横取りしているケースもあり得る（例: KDE Plasma、fcitx5 の Clipboard アドオン——既定で `<C-;>` にクリップボード履歴呼び出しが割り当てられていることがあり、Neovim にキーが届く前に消費されてしまう）。特定のキーが効かない場合は、端末側の符号化だけでなく、OS・デスクトップ環境・入力メソッド側のショートカット設定も確認すること。

[nvim-config-blink-skknvim](https://github.com/nabehan/nvim-config-blink-skknvim) では最終的に `enter_key = "<C-j>"`（既定のまま）、`extra_candidate_next_key = "<C-Left>"`/`extra_candidate_prev_key = "<C-Right>"` に落ち着いている（`<C-n>`/`<C-p>` 自体、ユーザーの手癖には手を加えていない）。Telescope側の `config.lua` は結局 `enter_key`（`<C-j>`）の上書きのみに戻し、`<C-n>`/`<C-p>` の上書きは削除している。
...
[nvim-autopairs](https://github.com/windwp/nvim-autopairs) 等、開き括弧・引用符に対応する閉じ文字を自動挿入するプラグインと併用すると、ひらがな/カタカナモードの `(`・`[`（`EXTRA_TARGET_CHARS` の変換対象キー）や全角英数モードの全記号で、確定済みの文字列の途中に新しい文字が割り込む不具合があった（例: `いろは` の直後に `(` を打つと `いろ()_は` のように、`は` の手前に割り込む）。

**原因調査の手法**: `nvim_feedkeys(keys, "x", false)` は Neovim の typeahead 処理全体が終わるまで `vim.schedule()` を一切実行しないため、この種の非同期タイミングに依存する不具合の再現には使えない（何を試しても意図通りに再現しない状態が続いた）。最終的に `nvim --headless --listen <socket>` でサーバーを起動し、別プロセスから `nvim --server <socket> --remote-send '<keys>'` で実際のキーストロークを1回のRPC呼び出しとして送り込む方式に切り替えたところ、実機と全く同じ症状を安定して再現できた。`--remote-send` は本物のキー入力に近い形で Neovim のイベントループに割り込むため、`vim.schedule()` の遅延実行やタイミング依存の不具合を検証する用途では、`feedkeys` より信頼できる。

**根本原因は2つ組み合わさっていた**:

1. nvim-autopairs は開き文字を打つと `<C-g>u()<C-g>U<Left><C-g>u`（Vim標準のアンドゥ境界制御イディオム）のような合成キー列を返す。開き文字・閉じ文字の両方が `process_romaji()` の変換対象キーである場合、同一ティック内でこの関数が複数回連続して呼ばれる。旧実装はキーが来るたびに独立して extmark を作成し、独立して `vim.schedule()` で書き込みを予約していたため、2回目の呼び出しが1回目の extmark を消して差し替えてしまい、さらに（Lua のクロージャが変数を参照で捕捉する性質により）先に実行される1回目の書き込みクロージャが本来2回目のものである extmark を誤って使用・消費するという競合が起きていた。
2. 合成キー列に含まれる `<Left>`（開き文字・閉じ文字の間へカーソルを戻すためのもの）は `capture.lua` 側では素通しされ Neovim ネイティブに処理されるが、対応する書き込みはまだ実行されていない（`vim.schedule` で1ティック遅延中）ため、まだ何も書き込まれていない古いバッファ状態に対してカーソルを動かしてしまう。`target.lua` の `M.replace_before_cursor()` は当時「extmark の位置が現在のカーソルより後ろなら不整合とみなしフォールバックする」というガードを持っていたため、この結果、正しい位置を指していたはずの extmark が信用されず、`<Left>` で動いた後のズレたカーソル位置へフォールバックしてしまっていた。

**修正（根本対応）**:

- `capture.lua` に「同一ティック内に蓄積された、まだ1回も flush されていないキー入力のまとまり」を1個のバッチとして扱う `hira_kata_batch`/`zenei_batch` という機構を新設した（全角英数モードは元々 extmark 追跡が皆無で最も脆弱だったため同様に新設）。バッチの最初のキーでのみ extmark 作成・`vim.schedule()` の予約を行い、後続のキーは単に蓄積するだけにすることで、同一ティック内で何回 `process_romaji()` が呼ばれても実際の書き込みは必ず1回になり、extmark 同士が競合する余地自体がなくなる。通常の（1キーずつ間隔をおいた）typing では、次のキーが来る前に前のバッチの flush が実行され終わっているため、実質的に「バッチサイズ1」となり従来と全く同じ挙動になる。
- `target.lua` の `M.replace_before_cursor()` は、削除範囲の終端も「現在のカーソル」ではなく「`start_pos`（extmark）から `byte_len` バイト進んだ位置」で計算するよう再設計し、現在のカーソルとの前後比較によるフォールバックを撤廃した。これにより開始・終端の両方が「実カーソル」から完全に独立し、`<Left>` 等の合間のカーソル移動に一切影響されなくなった。
- バッチが flush 待ちの間だけ `<Left>`/`<Right>` を横取りしてオフセット（文字数、`vim.fn.strcharpart()` でマルチバイト文字の境界を尊重）として蓄積し、flush 後にカーソルへ適用する仕組みを追加した。これにより、オートペアの `<Left>` が本来意図していた「開き文字と閉じ文字の間にカーソルを戻す」動きを、書き込みが1ティック遅延する設計のままで再現できる。
- 副次的に見つかった2件も同じ調査の過程で修正した。1つは、SKKの一時的全角記号入力（`z(` → `（` 等）の直後にオートペア対象の開き文字が続くと、オートペアの合成キー列先頭の `<C-g>` が「未知キーによる未確定バッファのリセット」を誤って発火させ、`z` の入力が確定を待っていた状態を消してしまう不具合（`z(` → `z()` になる）。`<C-g>` 自体もその場で処理を打ち切るよう修正した。もう1つは、`z(` の変換（`（`）に対してオートペアが自動追加する `)` が、`z` の効果が及ばない独立したキーとして処理されるため半角のまま（`（)`）になってしまう非対称性で、直前のキーで `z(` 変換が起きたことを示すワンショットフラグを追加し、直後の `)` に限り全角 `）` へ読み替えることで解消した。

**検証手法**: 上記の RPC 方式の再現に加え、`tests/*_spec.lua`（`test.sh`）による既存の回帰テスト（促音・`<Del>`/`<F11>` 誤発火防止・通常タイピング等）が壊れていないことを毎回確認しながら進めた。

**既知の限界**: マクロ等でローマ字入力と `<Left>`/`<Del>` を完全に同一ティックで送るような極端なケース（人間の実タイピングでは起こり得ない間隔）では、古いバッファ状態に対して動いてしまう影響が残ることを確認している。ただし現実的なキー間隔（実際のオートペア含む）には影響しない。「書き込みを1ティック遅延させる」というアーキテクチャ自体に内在する制限として許容している。

### abbrev モードでのオートペア相性問題と、推奨される回避策（実機で発見・重要）

上記の「nvim-autopairs との相性問題」を解決した後、v0.1.1 リリース後に別の症状が新規に報告された：abbrev モード（`/` から始める、見出しが ASCII 文字列そのものになるモード）で見出しが空、または辞書候補が0件の状態のまま `"` `'` `` ` `` `(` `[` `{` を打鍵すると、ライブ補完（blink.cmp 等）が起動せず、`()` のようなペア文字がプレーンテキストとしてそのまま即座に確定してしまう不具合があった（例: `<C-j>/(` のように `/` の直後に `(` を打つケースで顕著）。

**原因**: nvim-autopairs 等が生成する合成キー列 `<C-g>u()<C-g>U<Left><C-g>u` のうち、`<C-g>U` 直後の `<Left>`（開き文字・閉じ文字の間へカーソルを戻す、join_left イディオムの一部）が、blink.cmp のポップアップがまだ一度も表示されていない（`is_visible()==false`）間に `capture.lua` の「印字可能ASCIIでないキーが来たら、外部UIが見えていなければここまでの見出しを確定する」というフォールバックへ到達し、誤って `()` ごと即座に確定させてしまっていた。

**修正を試みたが実機で効果が確認できなかったアプローチ**: `<C-g>U` マーカー消費直後の1回に限り、続く `<Left>`/`<Right>` も無害化する `capture.lua` 側のパッチ（`pending_autopair_cursor_move`）を実装した。ヘッドレスNeovim（実機と同一の v0.12.4 バイナリ、`nvim --headless --listen` + `--remote-send` によるRPC経由キー入力再現）では症状の解消を確認できたが、**実機では同一のキー入力列・パッチ反映済みであることを確認し、Neovimを完全に再起動した上でも症状が解消しなかった**。原因は特定できていない（複数の `vim.on_key()` リスナーが同時に登録されている場合の相互干渉を疑い検証したが、再現性のある証拠は得られず否定）。オートペア側が生成する合成キー列を `capture.lua` 側で後追いに解釈するアプローチ自体、ヘッドレス環境と実機とで結果が食い違いうる、再現性・検証可能性の低い対応であることが分かったため、このパッチは revert 済み（skk.nvim 本体には変更を加えていない）。

**最終的に採用した解決策**: `capture.lua` 側でオートペアの合成キー列を解釈しようとするのではなく、**henkan（▽/▼/abbrev）がアクティブな間はオートペア側のプラグイン自体を丸ごと無効化し、`idle` に戻ったら再度有効化する**、という方針に転換した。skk.nvim は `▽`/`▼`/abbrev の状態変化のたびに `User autocmd "SkkHenkanChanged"` を `data.phase` 付きで発火するので（詳細は後述の「blink.cmp ネイティブソース統合」参照）、これをそのまま流用できる。skk.nvim 本体には一切変更を加えず、**ユーザー設定側だけで完結する**:
...
vim.api.nvim_create_autocmd("User", {
  ...
  callback = function(ev)
    ...
    -- ...（blink.cmp の show()/hide() 等、既存の phase 判定と同居させる）
  end,
})
```

henkan アクティブ中は abbrev だけでなく `▽`（midashi）・`▼`（select）も含めて対象にしている。`▽`（midashi）フェーズも abbrev と同じ `defer_to_external_ui` 判定を共有しており、理論上は同種の不具合が起こりうるため（実害が報告されたのは abbrev のみ）。

実機（[nvim-config-blink-skknvim](https://github.com/nabehan/nvim-config-blink-skknvim)）で push・確認済み: abbrev モードでの `"` `'` `` ` `` `(` `[` `{` への期待した挙動（プレエディット継続、見出しに単一の記号のみが入る）と、`▽`/`▼` を除いた通常入力（直接入力モード）でのオートペアによる閉じ記号補完が両立することを確認した。コマンドライン・検索モードはこの問題自体の対象外（元々オートペア系プラグインは通常 cmdline では動作しないため）で、従来通り問題なし。

副次的な効果として、この方式は「見出し(reading)に開き文字だけでなく自動挿入された閉じ文字までそのまま取り込まれてしまう」問題（例: `(` を打っても見出しが `"("` ではなく `"()"` になり、単体の記号として辞書引きできない）も同時に解消する。オートペア自体が henkan 中は動作しないため、閉じ文字がそもそも自動挿入されない。

#### 他のオートペア系プラグインでの enable/disable API 調査

上記の方式は `require("nvim-autopairs").enable()`/`.disable()` という、nvim-autopairs が提供する既存のグローバルトグルAPIを呼んでいるだけで、skk.nvim 側は特定のプラグインに依存していない。他のオートペア系プラグインでも、同種の enable/disable 相当のAPIさえあれば同じ方式が使えるはずと考え、代表的な3つを調査した（実装・実機検証はまだ行っておらず、ソースコード調査による机上確認のみ）。

| プラグイン                                                                  | API                                                                                                      | 備考                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [nvim-autopairs](https://github.com/windwp/nvim-autopairs)                  | `require("nvim-autopairs").enable()` / `.disable()` / `.toggle()`                                        | 採用・実機確認済み。グローバルな状態フラグ（`M.state.disabled`）を直接切り替えるシンプルな実装                                                                                                                                                                                                                                                                                                  |
| [mini.pairs](https://github.com/nvim-mini/mini.pairs)                       | `vim.g.minipairs_disable = true/false`（グローバル）または `vim.b.minipairs_disable`（バッファローカル） | 関数呼び出しではなく変数の代入で制御する設計。`SkkHenkanChanged` ハンドラ内で `vim.g.minipairs_disable = (phase ~= "idle")` のように書けば同様に実現できるはず                                                                                                                                                                                                                                  |
| [autoclose.nvim](https://github.com/m4xshen/autoclose.nvim)                 | `require("autoclose").toggle()` のみ                                                                     | 専用の `enable()`/`disable()` は無く、現在の状態を反転させる `toggle()` だけが公開されている。内部の状態フラグ（`config.disabled`）はモジュール外に公開されていないため、意図した状態に確実に持っていくには、こちら側で「今どちらの状態にしたか」を別途変数で追跡し、必要なときだけ `toggle()` を呼ぶ形にする必要がある（ユーザーが手動でも `toggle()` を使っていると状態がズレるリスクが残る） |
| [ultimate-autopair.nvim](https://github.com/altermo/ultimate-autopair.nvim) | `require("ultimate-autopair").enable()` / `.disable()` / `.toggle()` / `.isenabled()`                    | nvim-autopairs と同様、対称な enable/disable API を持つ。`isenabled()` で現在の状態も取得できる                                                                                                                                                                                                                                                                                                 |

nvim-autopairs 以外の3つについては実機での動作確認はまだ行っていない。特に autoclose.nvim は `toggle()` しか公開されていないため、他の3つと全く同じ書き方（`enable()`/`disable()` を無条件に呼ぶだけ）では正しく動作しない点に注意。
