...
- ~~未確定バッファがある状態で `<BS>` を押すと、画面上のテキストと内部状態 (`context.buffer`) がずれる~~ → **修正済み。** `<BS>` を明示的にインターセプトし、内部状態と画面を同期させるようにした。放置すると、次のキー入力時に「本来消すべきでないバイト数」を削除してしまい、既存の確定済みかな（マルチバイト文字）の末尾バイトだけを削って不正なバイト列に破壊してしまう実害のあるバグだった（実際に確認・修正済み）。
- ただし `<Left>`/`<Right>` などのカーソル移動キーや `<Delete>`、マウスクリックによるカーソル移動は、まだ同種のリスクが残っている。未確定バッファがある状態でこれらを使うと、内部状態と画面がずれる可能性がある（対処は今後の課題）。
