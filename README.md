...
**現在ステータス:** ローマ字→かな/カタカナ変換・4モード切替（切替時はカーソル位置にモードインジケーターを表示）に加えて、辞書変換（`▽`/`▼`、送りあり/送りなし/abbrev）・候補選択ウィンドウ（`<C-n>`/`<C-p>`フォーカス移動対応）・Sticky-shift・個人辞書（学習）・複数辞書のマージ・SKKサーバー連携に対応。大きな辞書ファイルは非同期・遅延パースで読み込む。挿入モードのバッファに加え、**コマンドラインモード**（`:`/`/`）でもローマ字→かな変換・4モード切替・モードインジケーター表示に対応（辞書変換 `▽`/`▼` はまだコマンドライン非対応、後述）。
...
**現時点の制約**: henkan（`▽`/`▼`、辞書変換）は `henkan/preedit.lua` の extmark ベースの表示に依存しており、extmark が存在しないコマンドラインにはまだ対応していない（ロードマップ参照）。そのため `capture.lua` は henkan・abbrev の開始トリガーをバッファ限定にガードしてあり、コマンドラインではローマ字→かな変換とモード切替のみが働く。
...
- 通常のバッファ（挿入モード）に加えて、コマンドラインモード（`c`）でもローマ字→かな変換・4モード切替・モードインジケーター表示に対応済み（`target.lua`、実機確認済み）。ただし辞書変換（`▽`/`▼`、henkan）はまだコマンドライン非対応（extmark ベースの preedit 表示に依存しているため。上記「コマンドラインモード対応」参照）。検索モード・内蔵ターミナル（`t`）は未対応。内蔵ターミナルはバッファへの直接書き込みができず `chansend()` でPTYにバイト列を送る形になり実装難易度が上がるほか、[skkeleton での実機検証](https://github.com/vim-skk/skkeleton)で確定した文字がターミナルに残らない不具合が見られたため、対応可否を含めて保留にしている。
...
13. コマンドラインモード・内蔵ターミナルへの対応
    - ~~ローマ字→かな変換・4モード切替・モードインジケーター表示~~ ✅（`target.lua`、実機確認済み）
    - 辞書変換（`▽`/`▼`、henkan）のコマンドライン対応（preedit をextmarkではなくコマンドライン文字列への直接書き込みに置き換える。候補選択ウィンドウの表示には `redraw` 強制が必要、上記参照）
    - 内蔵ターミナルへの対応は保留（skkeleton での実機検証で確定文字が残らない不具合が見られたため、可否を含めて先送り）
